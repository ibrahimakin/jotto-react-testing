...
export function useGuessedWords () {
    ...
    // useContext also subscribes to changes, and will update any time the context value
    //     we've memoized this so that it will only update when the guessedWords value update
    const context = React.useContext(guessedWordsContext);

    // throw an error if the context doesn't exist -- mean we aren't in a provider
    if (!context) {
...
export function GuessedWordsProvider (props) {
    ...
    // initial state value is empty array
